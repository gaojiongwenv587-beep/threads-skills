...
name: threads-skills
description: |
  Threads.net 全自動化操作技能。只要用戶提到 Threads 相關操作就觸發，包括：
  登錄/登出/檢查登錄/切換帳號/多帳號管理；
  發帖/發布/寫帖子/po文/發Thread/帶圖發帖；
  瀏覽首頁/刷一下/抓帖子/搜索/查帖子/看用戶主頁/爬取數據；
  點讚/按讚/轉發/回覆/評論/關注/互動；
  批量回覆/批量評論/逐條回覆/GUI回覆；
  競品分析/熱點追蹤/內容運營/推廣評論/定時抓帖/批量互動。
version: 1.0.0
metadata:
  openclaw:
    requires:
